...
- Save/Restore Laptop clone: `fnc_laptop_captureState`/`applyState` compare the exclusion set case-insensitively (`allVariables` returns lower-case names - the old mixed-case `in` test never matched, so non-serializable TEXT/CODE vars were captured and the apply loop aborted, leaving the userlist unset -> "Unknown user"/login timeout). Apply now wraps each `setVariable` in `try/catch` and re-binds networking with the restored parent/address (instead of resetting to loopback). Session vars are excluded so a clone starts signed out.

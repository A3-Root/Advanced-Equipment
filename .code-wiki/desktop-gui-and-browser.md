...
- Browser home: the address `home`/`rootnet` resolves to the seeded RootNet intel page (`rootnet.root`/`home.root`), falling back to the bundled Portal sample only if none is registered - not the sample by default. A bare `.md` link is page-relative when inside a mission site (`curDir` under `sites/`), else a wiki page.

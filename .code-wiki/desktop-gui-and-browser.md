...
- Web external apps are registered with `AE3_desktop_fnc_registerExtApp`; the web desktop replaces same-id external apps, removes absent external apps from later `ext_apps` pushes, and supports `deviceList` plus `launcher` template kinds.
...
- State-dependent external web apps should prefer `requiresFunction` when availability depends on mounted USBs or computed client state; addons can push a refreshed filtered `ext_apps` list after state changes.
...
- External app removal only applies to apps marked by the web extension registry (`external: true`); built-in apps are not removed by `ext_apps` refreshes.

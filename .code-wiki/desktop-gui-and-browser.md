...
- Browser mission pages: `A3.loadFile` searches the **mission root before** the mod PBO for CONTENT (`window.AE3_WEB_ROOTS = ["", WEB_ROOT]`), so a mission's `sites/portal/index.html` overrides the bundled sample. The `index.html` shell-asset loader uses a separate mod-first order (`[WEB_ROOT, ""]`) so bundled css/js do not spam "Script ... not found" mission-root misses in the RPT.
- Browser home: the address `home`/`rootnet` resolves to the seeded RootNet intel page (`rootnet.root`/`home.root`), falling back to the bundled Portal sample only if none is registered - not the sample by default. The page list (`web_pages`) loads asynchronously, so the first tab can render before it arrives; `reloadIntelPages()` re-navigates `home` once the list resolves (guarded on the active tab still showing `home`) so the RootNet home replaces the interim Portal flash. A bare `.md` link is page-relative when inside a mission site (`curDir` under `sites/`), else a wiki page.

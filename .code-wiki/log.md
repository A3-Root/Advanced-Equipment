...
2026-08-08 | updated | network-routing-and-ssh, multiplayer-locality-and-sync, filesystem-model, armaos-terminal, desktop-intel-and-communications | dedicated-server fixes + mail/chat export: (1) addressing made server-authoritative - connect_device2router/connect_router2router/removeNetworkConnection/dhcp_refresh/dhcp_onTurnOn self-route via new ae3_network_* server events (netId payloads, registered in network/XEH_preInit), dhcp_get/setStaticIp get hard isServer exits + WARNING tripwires; ae3_desktop_computers now broadcast so client-reached ipInUse stops always returning false (root cause of duplicate IPs from the ACE connect menu); ipInUse also rejects router gateway addresses. (2) every server->client web reply now goes through new AE3_desktop_fnc_routeReply (CBA_fnc_encodeJSON on the wire, parseJSON mode 2 client-side) instead of sending HashMaps over CBA events - fixes net_setip/mail_send/chat_send/handle+addr create-delete/ssh ops silently never resolving on dedicated; ae3_desktop_sshReply folded into ae3_desktop_routeReply and removed. (3) Zeus/desktop raw remoteExecCall ["setVariable",2] replaced with named fns AE3_armaos_fnc_computer_setHostname / AE3_network_fnc_setSshEnabled; new AE3_network_fnc_setStaticIpZeus reports the server's real verdict to the curator (ae3_network_zeusIpResult); zeus_initAttributes waitUntil on AE3_power_initDone bounded, router picker unions AE3_network_routers. (4) sudoers made replication-safe: new broadcast AE3_sudoers object var written by addSudoer/removeSudoer and seeded in device_initComplete, getSudoers returns roster+file union, isSudoer trims and case-folds - fixes GUI "Missing Permissions" for a valid sudoer on a stale/mid-sync client filesystem. (5) new `su [user]` command (fnc_os_su, /bin/su) with AE3_terminalSuStack; os_exit pops it before logging out; onUnload drops back to the original account and clears the stack; sudo restore now conditional. (6) Mail/Messenger export + OS clipboard copy - formatMail/formatThread/fileSlug/exportText in apps.js reuse AE3_pickFile("save")+fs_save, new window.AE3_copyText in desktop.js (clipboard API -> execCommand -> manual Ctrl+C dialog); removed the server-restricted copyToClipboard call from fnc_encryption_columnar
